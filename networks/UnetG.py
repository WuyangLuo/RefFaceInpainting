...
class UnetG(nn.Module):
    ...
    # generte a style map only contain face component region
    def mask_region(self, segmap, style_map, mask):
        bs, D, _, _ = style_map.size()
        _, N, h, w = segmap.size()
        mask = F.interpolate(mask, size=segmap.size()[2:], mode='nearest')
        # select pre-defined face components for extracting texture
        region_select = [2, 3, 4, 5, 12, 13]
        for j in range(N):  # N: number of semantic class
            if j not in region_select:  # non-component class
                style_map = style_map * (1.0 - segmap[:, j:j + 1])  # set this class to zero
        style_map_select = style_map.clone()

        # delete the components that appear in non-masked region
