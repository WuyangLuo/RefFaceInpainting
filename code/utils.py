...
def generate_draw(mask, maks_name=None):
    ...
    while olrate <= th_olrate:
        ...
        vertex.append((pointX[startidx], pointY[startidx]))  # 设置起点
        num = 0
        while num < num_vertex:
            if num % 2 == 0:
                angle = 2 * math.pi - np.random.uniform(angle_min, angle_max)  # 反向画线
            else:
...
# def tensor2im(input_image, imtype=np.uint8, no_fg=True):
    ...
    # if not isinstance(input_image, np.ndarray):
        ...
        # if no_fg:
            ...
            # image_numpy = (image_numpy * [0.229, 0.224, 0.225] + [0.485, 0.456, 0.406]) * 255
        # else:
            # image_numpy = np.transpose(image_numpy, (1, 2, 0))
            # image_numpy = (image_numpy * [0.229, 0.224, 0.225] + [0.485, 0.456, 0.406]) * 255
        # image_numpy = np.clip(image_numpy, 0, 255)
...
# def tensor2im(input_image, imtype=np.uint8, no_fg=True):
    ...
    # if not isinstance(input_image, np.ndarray):
        ...
        # else:
            # # image_numpy = (np.transpose(image_numpy, (1, 2, 0))) * 255.0
            # image_numpy = (image_numpy[0]) * 255.0
        # image_numpy = np.clip(image_numpy, 0, 255)
    # else:  # if it is a numpy array, do nothing
        # image_numpy = input_image
