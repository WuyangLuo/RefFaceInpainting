...
class AdaIN(nn.Module):
    ...
    def forward(self, input, z_id):
        gamma = self.fc_1(z_id).unsqueeze(-1).unsqueeze(-1).expand_as(input)
        beta = self.fc_2(z_id).unsqueeze(-1).unsqueeze(-1).expand_as(input)

        in_mean, in_var = torch.mean(input, dim=[2, 3], keepdim=True), torch.var(input, dim=[2, 3], keepdim=True)
        out_in = (input - in_mean) / torch.sqrt(in_var + self.eps)
        out = out_in * gamma + beta

        return out

class AdaINResBlock(nn.Module):
    def __init__(self, h_inchannel, h_outchannel):
    ...
    def forward(self, h_in, z_id):
        x1 = self.conv1(h_in)
        x1 = self.activation(self.adain_1(x1, z_id))
        x1 = self.conv2(x1)
        x1 = self.adain_2(x1, z_id)

        return x1 + h_in


class GatedAdaINResBlock(nn.Module):
    def __init__(self, h_inchannel, h_outchannel):
        super(GatedAdaINResBlock, self).__init__()
        ...
        self.inpainting_conv_1 = Conv2dBlock(h_inchannel, h_inchannel, kernel_size=3, stride=1, padding=1, norm='in',
                                             activation='relu')
        self.inpainting_conv_2 = Conv2dBlock(h_inchannel, h_inchannel, kernel_size=3, stride=1, padding=1, norm='in',
                                             activation='none')

        self.mask_conv_1 = nn.Conv2d(h_inchannel, h_inchannel, 3, stride=1, padding=1)
        self.mask_conv_2 = nn.Conv2d(h_inchannel, h_outchannel, 3, stride=1, padding=1)

        self.activation = nn.ReLU()
        self.sigmoid = torch.nn.Sigmoid()

    def forward(self, h_in, z_id):
        x1_adain = self.conv1(h_in)
        x1_adain = self.activation(self.adain_1(x1_adain, z_id))
        x1_inpainting = self.inpainting_conv_1(h_in)
        mask_1 = self.sigmoid(self.mask_conv_1(h_in))
        x1 = x1_inpainting*(1-mask_1) + x1_adain*mask_1

        x2_adain = self.conv2(x1)
        x2_adain = self.activation(self.adain_2(x2_adain, z_id))
        x2_inpainting = self.inpainting_conv_2(x1)
        mask_2 = self.sigmoid(self.mask_conv_2(x1))
        x2 = x2_inpainting*(1-mask_2) + x2_adain*mask_2

        return x2 + h_in, mask_1, mask_2
...
class IDGatedConv2d(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv_in = Conv2dBlock(in_channels, out_channels, kernel_size=3, stride=1, padding=1, norm='none', activation='none')

        self.fc_1 = nn.Linear(512, out_channels//2)
        self.fc_2 = nn.Linear(512, out_channels//2)
        
        self.gated_conv2d = nn.Conv2d(out_channels//2, out_channels//2, kernel_size=3, stride=1, padding=1)
        self.sigmoid = torch.nn.Sigmoid()
        
        self.activation = nn.LeakyReLU(0.2)
        
        # self.conv_x = Conv2dBlock(in_channels, out_channels, kernel_size=3, stride=1, padding=1, norm='in', activation='lrelu')
        
        self.in_norm = nn.InstanceNorm2d(out_channels//2)
