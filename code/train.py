...
import numpy as np
import datetime
from trainer import BiMod_Trainer
...
for epoch in range(cur_epoch, cfg['max_epoch']):
        ...
        if i % cfg['visual_img_freq'] < visual_iter_num:  # display images on visdom and save images to a HTML file
            ...
            for name, img in results.items():
                ...
                if name == 'lab':
                    ...
                    # cv2.imwrite(os.path.join(cur_img_dir, save_name), cv2.applyColorMap(tensor2hm(img), cv2.COLORMAP_JET))
                    ...
                    # tensor2im(img, no_fg=no_fg).save(os.path.join(cur_img_dir, save_name))
    ...
    # test
    # ref_gt_arcface_loss = []
    # fake_gt_arcface_loss = []
    # fake_ref_arcface_loss = []
    if epoch % cfg['test_freq'] == 0:
        with torch.no_grad():
            ...
            for i, data in enumerate(test_loader):
                ...
                results, img_name = trainer.visual_results()
                # if not os.path.exists(os.path.join(cur_save_dir, img_name[0]+'.jpg')):
                    # os.makedirs(os.path.join(cur_save_dir, img_name[0]+'.jpg'))
                cv2.imwrite(os.path.join(cur_save_dir, img_name[0]+'.jpg'), tensor2im(results['masked_fake']))

            #     # arcface loss
            #     ref_gt, fake_gt, fake_ref = trainer.compute_arcface_loss()
            #     ref_gt = ref_gt.detach().cpu().float().numpy()
            #     fake_gt = fake_gt.detach().cpu().float().numpy()
            #     fake_ref = fake_ref.detach().cpu().float().numpy()
            #
            #     ref_gt_arcface_loss.append(ref_gt)
            #     fake_gt_arcface_loss.append(fake_gt)
            #     fake_ref_arcface_loss.append(fake_ref)
            #
            # print('arcface loss ref_gt: ', np.mean(ref_gt_arcface_loss))
            # print('arcface loss fake_gt: ', np.mean(fake_gt_arcface_loss))
            # print('arcface loss fake_ref: ', np.mean(fake_ref_arcface_loss))
